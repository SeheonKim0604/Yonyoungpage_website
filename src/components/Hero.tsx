...
export default function Hero() {
  ...
  if (isLoading) return <section className="hero loading-hero"></section>
    ...
    <section className="hero">
        ...
        <div className="hero-title-wrapper">
          {heroData?.backgroundImage && (
            <div className="hero-title-bg">
              <Image src={heroData.backgroundImage} alt="배경 로고" width={400} height={400} style={{ objectFit: 'contain' }} />
            </div>
          )}
          <h1>연영회</h1>
          {isAdminMode && (
            <button className="hero-edit-btn" onClick={() => fileInputRef.current?.click()} title="배경 이미지 수정">✎</button>
          )}
          <input type="file" ref={fileInputRef} onChange={handleHeroImageUpload} style={{ display: 'none' }} accept="image/*" />
        </div>
      ...
      <div className="hero-exhibition-section">
        ...
        <div className="hero-main-container">
          <button className="hero-nav-btn prev" onClick={handlePrev} aria-label="이전 이미지">‹</button>
              ...
              {exhibitionImages.length > 0 ? (
                exhibitionImages.map((image: string, index: number) => (
                  <div key={index} className={`hero-slide ${index === currentImageIndex ? 'active' : ''}`}>
                    <Image src={image} alt={`${latestExhibition?.title} - 이미지 ${index + 1}`} fill priority={index === 0} style={{ objectFit: 'contain' }} sizes="(max-width: 1200px) 100vw, 1200px" />
                  </div>
                ))
              ) : (
                <div className="hero-placeholder"><p>전시 이미지가 없습니다.</p></div>
              )}
            </div>
          </div>
          <button className="hero-nav-btn next" onClick={handleNext} aria-label="다음 이미지">›</button>
        </div>

        {exhibitionImages.length > 1 && (
          <div className="hero-indicators">
            {exhibitionImages.map((_: any, index: number) => (
              <button key={index} className={index === currentImageIndex ? 'active' : ''} onClick={() => handleIndicatorClick(index)} aria-label={`배너 ${index + 1}로 이동`} />
            ))}
          </div>
        )}

        <div className="hero-exhibition-footer">
          <Link href="/archive/exhibitions" className="view-all-exhibitions-btn">모든 전시 보기 →</Link>
        </div>
      </div>
    </section>
  )
